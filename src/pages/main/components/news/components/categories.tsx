        ...
        <div className="filter-top-cards-categories__items">

            {
                servers.map((item: IServers) =>
                    <div onClick={_ => setServer(item)} style={{display: news.some(item2 => item2.server.id === item?.id) ? "block" : "none"}} className={"filter-top-cards-categories__item" + (item.id === server?.id ? " active" : "")}>
